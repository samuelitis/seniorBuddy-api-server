...
async def create_medication_time(medication_time: MedicationTimeCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
    복약 시간을 설정하는 API.
...
async def get_medication_times(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
    복약 시간을 조회하는 API.
    user_id: 사용자의 ID
...
async def get_medication_time(medication_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
    특정 복약 시간을 조회하는 API.
...
async def update_medication_time(medication_id: int, medication_time: MedicationTimeUpdate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
    복약 시간을 수정하는 API.
...
async def delete_medication_time(medication_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
    복약 시간을 삭제하는 API.
...
async def set_medication_reminder(medication_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
    복약 알림 설정 API.
